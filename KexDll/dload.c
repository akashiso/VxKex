	...
	IN	ULONG								Flags)
{
	...
	Status = LdrGetProcedureAddress(KexData->SystemDllBase, &LdrResolveDelayLoadedAPIName, FALSE, (PPVOID) &pLdrResolveDelayLoadedAPI);
	if (NT_SUCCESS(Status) && pLdrResolveDelayLoadedAPI) {
