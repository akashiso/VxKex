...
typedef enum _KEX_VTBL_REPLACING_MODE
{
	KEX_VTBL_REPLACING_ALL,
	KEX_VTBL_REPLACING_EXTERNAL_ONLY,
	KEX_VTBL_REPLACING_MAXIMUM
} KEX_VTBL_REPLACING_MODE;

typedef struct _KEX_VTBL_MODIFICATION
{
	SIZE_T ByteOffset;
	PVOID  Function;
} TYPEDEF_TYPE_NAME(KEX_VTBL_MODIFICATION);

typedef struct _KEX_VTBL_REPLACING_ENTRY
{
	SIZE_T ByteOffset;
	PVOID  Function;
	KEX_VTBL_REPLACING_MODE Mode;
} TYPEDEF_TYPE_NAME(KEX_VTBL_REPLACING_ENTRY);

typedef struct _KEX_VTBL_WRAPPER
{
	PPVOID lpVtbl;
	PVOID  This;
} TYPEDEF_TYPE_NAME(KEX_VTBL_WRAPPER);

typedef struct _KEX_VTBL_REPLACEMENT_WRAPPER
{
	PVOID pContext;
	...
	PPVOID OriginalVtbl;
	PPVOID ModifiedVtbl;
} TYPEDEF_TYPE_NAME(KEX_VTBL_REPLACEMENT_WRAPPER);


typedef struct _KEX_VTBL_REWRITE_DATA
{
	RTL_DYNAMIC_HASH_TABLE_ENTRY HashTableEntry;
