...
typedef enum _KEX_VFT_REPLACING_MODE
{
	KEX_VFT_REPLACING_ALL,
	KEX_VFT_REPLACING_EXTERNAL_ONLY,
	KEX_VFT_REPLACING_MAXIMUM
} KEX_VFT_REPLACING_MODE;

typedef struct _KEX_VFT_VTBL_MODIFICATION
{
	SIZE_T ByteOffset;
	PVOID  Function;
} TYPEDEF_TYPE_NAME(KEX_VFT_VTBL_MODIFICATION);

typedef struct _KEX_VFT_REPLACING_ENTRY
{
	SIZE_T ByteOffset;
	PVOID  Function;
	KEX_VFT_REPLACING_MODE Mode;
} TYPEDEF_TYPE_NAME(KEX_VFT_REPLACING_ENTRY);

typedef struct _KEX_VFT_WRAPPER
{
	PPVOID lpVtbl;
	PVOID  This;
} TYPEDEF_TYPE_NAME(KEX_VFT_WRAPPER);

typedef struct _KEX_VFT_REPLACEMENT_WRAPPER
{
	PVOID pBundles;
	PVOID ModuleAddrStart;
	PVOID ModuleAddrEnd;
	PPVOID OriginalVtbl;
	PPVOID ModifiedVtbl;
} TYPEDEF_TYPE_NAME(KEX_VFT_REPLACEMENT_WRAPPER);


typedef struct _KEX_VFT_REWRITE_DATA
{
	RTL_DYNAMIC_HASH_TABLE_ENTRY HashTableEntry;

	PPVOID OriginalVFTable;
	PPVOID RewrittenVFTable;

	SIZE_T NumberOfFuncs;
	UINT   RefCount;
} TYPEDEF_TYPE_NAME(KEX_VFT_REWRITE_DATA);
...
#pragma region KexVtbl* functions
...
KEXAPI BOOLEAN NTAPI KexVtblReplace(
	...
	IN	PKEX_VFT_REPLACING_ENTRY	Entry,
	IN  PVOID   RefVtbl,
	IN  UINT    NumberOfEntry,
