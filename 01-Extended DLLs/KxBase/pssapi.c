...
KXBASEAPI DWORD WINAPI PssWalkMarkerGetPosition(
	IN	HPSSWALK		WalkMarkerHandle,
	OUT ULONG_PTR*		Position)
{
	KexLogWarningEvent(L"Stub API called: PssWalkMarkerGetPosition");
	KexDebugCheckpoint();
	return ERROR_NOT_SUPPORTED;
}

KXBASEAPI DWORD WINAPI PssWalkMarkerSetPosition(
	IN	HPSSWALK		WalkMarkerHandle,
	IN  ULONG_PTR		Position)
{
	KexLogWarningEvent(L"Stub API called: PssWalkMarkerSetPosition");
	KexDebugCheckpoint();
	return ERROR_NOT_SUPPORTED;
}

KXBASEAPI DWORD WINAPI PssWalkMarkerSeekToBeginning(
	IN	HPSSWALK		WalkMarkerHandle)
{
	KexLogWarningEvent(L"Stub API called: PssWalkMarkerSeekToBeginning");
	KexDebugCheckpoint();
	return ERROR_NOT_SUPPORTED;
}

KXBASEAPI DWORD WINAPI PssWalkMarkerFree(
	IN	HPSSWALK		WalkMarkerHandle)
{
	KexLogWarningEvent(L"Stub API called: PssWalkMarkerFree");
	KexDebugCheckpoint();
	return ERROR_NOT_SUPPORTED;
}