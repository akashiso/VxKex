	...
	OUT	PVOID					*dispatcherQueueController)
{
	...
	if (options.threadType == DQTYPE_THREAD_DEDICATED)
	{
		DispatcherQueueCreateOnNewThread(&dq);
	}
	else if (options.threadType == DQTYPE_THREAD_CURRENT)
	{
		if (DispatcherQueueGetForThread(GetCurrentThreadId()))
			return E_FAIL;

		DispatcherQueueCreateOnThread(GetCurrentThreadId(), &dq);
	}
...
STATIC RTL_SRWLOCK eventHandlerSRWLock = { 0 };
STATIC EventRegistrationToken nextToken = { 1 };

HRESULT EventHandlersAppend(
	OUT IEventHandlerEntry** list, 
	IN  ITypedEventHandler* handler, 
	OUT EventRegistrationToken* token)
{
	IEventHandlerEntry* entry;
	entry = (IEventHandlerEntry*)CoTaskMemAlloc(sizeof(IEventHandlerEntry));
	if (entry == NULL)
		return E_OUTOFMEMORY;
	...
	IN  EventRegistrationToken* token)
{
	...
	IEventHandlerEntry* cur = *list;
	IEventHandlerEntry* prev = NULL;
	while (cur != NULL)
	{
		if (token->value == cur->token.value)
			break;
		prev = cur;
		cur = cur->prev;
	}
...
void EventHandlersNotify(
	IEventHandlerEntry* list, 
	IInspectable* thiz)
{
	RtlAcquireSRWLockShared(&eventHandlerSRWLock);
	...
	RtlReleaseSRWLockShared(&eventHandlerSRWLock);
}